...
func rowFromString(input string) []int {
	step1 := strings.Split(input, " ")
	output := make([]int, len(step1))

	for i, e := range step1 {
		output[i], _ = strconv.Atoi(e)
	}

	return output
}

func getHighLow(input []int) (high int, low int) {
	high = input[0]
	low = input[0]
...
func getEvenlyDivisable(input []int) (high int, low int) {
	sort.Ints(input)

	for i, l := range input {
		for _, h := range input[i+1:] {
			if h%l == 0 {
				return h, l
			}
		}
...
func normalize(input string) string {
	step1 := strings.Replace(input, `	`, ` `, -1)
	step2 := strings.Trim(step1, `	`)
	return strings.Trim(step2, ` `)
}

func checksum1(scanner *bufio.Scanner) (output int) {
	for scanner.Scan() {
		normalized := normalize(scanner.Text())
...
func checksum2(scanner *bufio.Scanner) (output int) {
	for scanner.Scan() {
		normalized := normalize(scanner.Text())
		row := rowFromString(normalized)
		high, low := getEvenlyDivisable(row)
		output += high / low
	}
	return
}
...
func main() {
	...
	input1 := `414	382	1515	319	83	1327	116	391	101	749	1388	1046	1427	105	1341	1590
	...
	fmt.Println("input 1 : ", checksum1(bufio.NewScanner(strings.NewReader(input1))))
	fmt.Println("input 2 : ", checksum2(bufio.NewScanner(strings.NewReader(input1))))
}
